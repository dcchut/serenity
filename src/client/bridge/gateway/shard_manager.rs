...
impl ShardManager {
    ...
    pub async fn shutdown(&mut self, shard_id: ShardId) -> bool {
        ...
        if let Some(runner) = self.runners.lock().await.get_mut(&shard_id) {
            ...
            if let Err(why) = runner.runner_tx.send(msg).await {
                warn!(
            ...
            while let Some(shutdown_shard_id) = self.shard_shutdown.next().await {
    ...
    pub async fn shutdown_all(&mut self) {
        ...
        let _ = self.shard_queuer.send(ShardQueuerMessage::Shutdown).await;
        let _ = self.monitor_tx.send(ShardManagerMessage::ShutdownInitiated).await;
    }
