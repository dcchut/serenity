...
impl ShardRunner {
    ...
    async fn handle_rx_value(&mut self, value: InterMessage) -> bool {
        match value {
            InterMessage::Client(value) => match *value {
                ShardClientMessage::Manager(ShardManagerMessage::Start(_, _)) => true,
    ...
    async fn recv(&mut self) -> Result<bool> {
        ...
        let next = timeout(Duration::from_millis(50), self.runner_rx.next());
    ...
    async fn update_manager(&mut self) {
        let _ = self
            .manager_tx
            .send(ShardManagerMessage::ShardUpdate {
